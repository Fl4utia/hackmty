...
interface Requirement {
  ...
  price: number;
  paid: boolean;
  paidBy?: string; // Assuming user ID of the person who paid
}
...
const mockEvent: Event = {
  name: "Sample Event",
  products: [
    { name: "Product 1", units: 2, price: 10, paid: false, paidBy: "Participant 1" },
    { name: "Product 2", units: 1, price: 20, paid: true, paidBy: "Participant 2" },
  ],
  equitative: false, // Change this to true to test the other functionality
  admin: "adminUserId",
  participants: ["Participant 1", "Participant 2"], // Add some participants for demonstration
  members: [
    { user: "Participant 1", paid: 20 },
    { user: "Participant 2", paid: 10 },
  ],
};
              ...
              <thead>
                <tr>
                  ...
                  {event?.equitative ? (
                    ...
                    <th className="border border-gray-300 px-4 py-2">Products Paid</th>
                  )}
                </tr>
              </thead>
              <tbody>
                {event?.members?.map((member, index) => (
                  <tr key={index} className="text-center">
                    <td className="border border-gray-300 px-4 py-2">
                      {member.user} {/* Display the member's name */}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                    ...
                    {event?.equitative ? (
                      ...
                      <td className="border border-gray-300 px-4 py-2">
                        {event?.products
                          .filter((product) => product.paidBy === member.user)
                          .map((product, index) => (
                            <div key={index}>
                              {product.name} (${(product.price * product.units).toFixed(2)})
                            </div>
                          ))}
                      </td>
                    )}
